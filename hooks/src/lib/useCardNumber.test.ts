...
describe("useCardNumber 테스트", () => {
  describe("change 상태일 때,", () => {
    test("정상 입력시, 에러를 발생시키지 않는다.", () => {
      ...
      expect(flatten(result.current.states, "isError")).toEqual([false, false, false, false]);
    });
    test("첫 번째 인풋태그에 문자를 입력했을 때, 첫번째 입력태그에만 에러를 발생시킨다.", () => {
      ...
      expect(flatten(result.current.states, "isError")).toEqual([true, false, false, false]);
    });
  });
  describe("blur 상황일 때,", () => {
    ...
    test.each([0, 1, 2, 3])(
      ...
      (index: number) => {
        ...
        expect(flatten(result.current.states, "isError")).toEqual(EXPECTED_RESULT);
      }
    );
  });
});
